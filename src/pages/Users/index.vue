<template>
    <div>
        <crud-table-component :tableData="tableData" @createItem="showFormCreateUser" @editItem="showFormEditUser"
            @deleteItem="deleteUser" class="mx-5 my-5" />
    
        <create-or-edit-modal-component :formDetails="formDetails" @createOrUpdateItem="createOrUpdateUser">
            <template v-slot:form>
                <v-text-field v-model="formData.name" label="Full Name" :rules="[rules.required]" oultined />
    
                <v-text-field v-model="formData.email" type="email" label="Email Address"
                    :rules="[rules.required, rules.email]" oultined />
    
                <v-text-field v-if="formDetails.action === 'Create'" v-model="formData.password" type="password"
                    label="Password" :rules="[rules.required]" oultined />
    
                <v-select v-model="formData.role_id" label="Role" :rules="[rules.required]" :items="roles" item-text="name"
                    item-value="id" />
            </template>
